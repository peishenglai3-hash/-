...
按 `P` 打开可拖动的 `ZONE FORGE` 面板。碰撞箱为粉色、交互区为黄色，支持新增、重命名、精确输入、拖动及从四边/四角缩放，面板打开时角色仍可移动。

“前景套索”用于从底图自由框选需要遮住角色的区域：点击“开始套索”，在画布按住鼠标绘制并松开完成，`Esc` 取消。选中套索后可整体拖动，修改 `Depth` 会实时生效；点击“保存 JSON”写回场景数据。场景一推荐保持默认 `Depth 2000`，场景二保持默认 `Depth 100`。
